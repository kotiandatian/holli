...
public final class SettingUtils {
	...
	public static Setting get() {
		...
		if (cacheElement != null) {
			...
			setting = new Setting();
			try {
				File shopxxXmlFile = new ClassPathResource(CommonAttributes.FRAMEWORK_XML_PATH).getFile();
				Document document = new SAXReader().read(shopxxXmlFile);
				List<Element> elements = document.selectNodes("/framework/setting");
	...
	public static void set(Setting setting) {
		try {
			File frameworkXmlFile = new ClassPathResource(
					CommonAttributes.FRAMEWORK_XML_PATH).getFile();
			Document document = new SAXReader().read(frameworkXmlFile);
			List<Element> elements = document.selectNodes("/framework/setting");
			for (Element element : elements) {
				try {
					String name = element.attributeValue("name");
			...
			try {
				...
				fileOutputStream = new FileOutputStream(frameworkXmlFile);
