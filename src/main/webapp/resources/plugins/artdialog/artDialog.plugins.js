 ...
 * @param	{Number}	显示时间 (默认1.5秒)
 */
$.message = $.dialog.message = function (message, time) {
	if (typeof(message) != "undefined" && message != null) {
		 if(typeof message == 'object'){
			if(message.type == 'success'){
				art.success(message.content,time);
			}else if(message.type == 'warn'){
				art.warn(message.content,time);
			}else if(message.type == 'error'){
				art.error(message.content,time);
			}
		 }else if(typeof message == 'string'){
			 art.tips(message,time);
		 }
	}
};
 ...
 * @param	{Number}	显示时间 (默认1.5秒)
 */
$.error = $.dialog.error = function (content, time) {
    return $.dialog({
    	id: 'TipsError',
		...
		fixed: true,
		lock: false
    }).content('<div><img src="'+framework.base+'/resources/plugins/artdialog/skins/icon/error.png" style="cursor: auto;"/><span style="display: block;float: right;font-size: 17px;height: 50px;line-height: 55px;padding-left: 15px;">' + content + '</span></div>')
	.time(time || 3000);
};
 ...
 * @param	{Number}	显示时间 (默认1.5秒)
 */
$.warn = $.dialog.warn = function (content, time) {
    return $.dialog({
    	id: 'TipsWarnning',
    	padding:'3px 20px 3px 5px',
		title: false,
		...
		fixed: true,
		lock: false
    }).content('<div><img src="'+framework.base+'/resources/plugins/artdialog/skins/icon/warning.png" style="cursor: auto;"/><span style="display: block;float: right;font-size: 17px;height: 50px;line-height: 55px;padding-left: 15px;">' + content + '</span></div>')
	.time(time || 3000);
};
 ...
 * @param	{Number}	显示时间 (默认1.5秒)
 */
$.success = $.dialog.success = function (content, time) {
    return $.dialog({
    	id: 'TipsSuccess',
    	padding:'3px 20px 3px 5px',
		title: false,
		...
		fixed: true,
		lock: false
    }).content('<div><img src="'+framework.base+'/resources/plugins/artdialog/skins/icon/success.png" style="cursor: auto;"/><span style="display: block;float: right;font-size: 17px;height: 50px;line-height: 55px;padding-left: 15px;">' + content + '</span></div>')
	.time(time || 3000);
};


$.loading = $.dialog.loading = function (content, time) {
    return $.dialog({
    	id: 'TipsLoading',
    	padding:'3px 20px 3px 5px',
		title: false,
		...
		fixed: true,
		lock: false
    }).content('<div><img src="'+framework.base+'/resources/plugins/artdialog/skins/icon/loading.gif" style="cursor: auto;"/><span style="display: block;float: right;font-size: 17px;height: 50px;line-height: 55px;padding-left: 15px;">' + content + '</span></div>')
	.time(time || 3000);
};
